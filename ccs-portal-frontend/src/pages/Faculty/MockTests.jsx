    ...
    <Layout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 transition-colors duration-300">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div>
            <h1 className="text-4xl font-black text-gray-900 dark:text-white uppercase tracking-tightest mb-2 transition-colors">Mock Assessments</h1>
            <p className="text-sm font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest leading-relaxed transition-colors">Design, manage, and monitor your specialized evaluation blueprints.</p>
          </div>
          <button 
            onClick={() => navigate('/faculty/test-builder')}
            className="flex items-center gap-3 px-8 py-5 bg-indigo-600 text-white rounded-[2rem] font-black text-xs uppercase tracking-[0.2em] hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100 dark:shadow-indigo-900/20 group"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
           <div className="bg-[#0f172a] dark:bg-[#1e293b] rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl transition-colors">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 mb-2">Total Managed</p>
              <h3 className="text-4xl font-black">{tests.length}</h3>
              <div className="flex items-center gap-2 mt-4 text-emerald-400">
                 <TrendingUp className="w-4 h-4" />
                 <span className="text-xs font-bold uppercase tracking-widest">Active Curriculum</span>
              </div>
           </div>
           ...
           <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 border border-gray-100 dark:border-slate-700 shadow-sm transition-colors">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 dark:text-gray-500 mb-2">Avg. Duration</p>
              <h3 className="text-4xl font-black text-gray-900 dark:text-white transition-colors">
                 {tests.length > 0 ? Math.round(tests.reduce((acc, t) => acc + (t.duration || 0), 0) / tests.length) : 0}
                 <span className="text-xl ml-2">min</span>
              </h3>
              <p className="text-xs font-bold text-gray-500 dark:text-gray-400 mt-4 uppercase tracking-widest leading-none transition-colors">Per evaluation</p>
           </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-slate-800/50 rounded-[4rem] border-2 border-dashed border-gray-100 dark:border-slate-700 p-32 text-center shadow-inner transition-colors">
             ...
             <h3 className="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tight mb-2 transition-colors">No blueprinted tests</h3>
             ...
             <button 
               onClick={() => navigate('/faculty/test-builder')}
               className="px-10 py-4 bg-gray-900 dark:bg-white dark:text-gray-900 text-white rounded-3xl font-black text-[10px] uppercase tracking-widest hover:bg-black dark:hover:bg-gray-100 transition-all"
             >
            ...
            {filteredTests.map((test) => (
              ...
              >
                ...
                <div className="mb-6 flex flex-wrap items-center gap-3">
                   <span className="px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400">
                      {test.category}
                   </span>
                   {test.isProctored && (
                     <span className="flex items-center gap-2 px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400">
                ...
                <h4 className="text-2xl font-black text-gray-900 dark:text-white mb-8 font-sans leading-tight pr-20 transition-colors uppercase">{test.title}</h4>
                
                <div className="grid grid-cols-3 gap-4 mb-10 pb-8 border-b border-gray-50 dark:border-slate-700/50">
                   <div>
                      <p className="text-[9px] font-black text-gray-300 dark:text-gray-600 uppercase tracking-widest mb-1">Time Limit</p>
                   ...
                   <div>
                      <p className="text-[9px] font-black text-gray-300 dark:text-gray-600 uppercase tracking-widest mb-1">Questions</p>
                      <div className="flex items-center gap-2">
                   ...
                   <div>
                      <p className="text-[9px] font-black text-gray-300 dark:text-gray-600 uppercase tracking-widest mb-1">Max Marks</p>
                      <div className="flex items-center gap-2">
                ...
                <div className="flex items-center justify-between">
                   <div className="flex items-center gap-3">
                      ...
                      <div>
                         <p className="text-[9px] font-black text-gray-300 dark:text-gray-600 uppercase tracking-widest leading-none">Blueprint Date</p>
                         <p className="text-xs font-bold text-gray-500 dark:text-gray-400 mt-1">{new Date(test.createdAt).toLocaleDateString()}</p>
                      </div>
                   </div>
                   <button 
                     onClick={() => navigate(`/faculty/test-builder/${test._id}`)}
                     className="flex items-center gap-2 px-6 py-3 bg-gray-900 dark:bg-white dark:text-gray-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-black dark:hover:bg-gray-100 transition-all shadow-lg"
                   >
                      Modify Blueprint
